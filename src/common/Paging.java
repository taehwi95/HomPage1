...
public class Paging {
	...
	public static String getPaging(String path, int page, int recordPerPage, int totalRecord) {
		 ...
		 * 		2) 이전 버튼의 링크가 필요한 경우 : 위 상황 이외의 경우
		 */
		if(beginPage < pagePerBlock) {
			sb.append("<span style='color: lightgray;'>◀</span>&nbsp;&nbsp;");
		} else {
			if(path.indexOf("?") > 0) {
				sb.append("<a style='text-decoration:none; color: lightgray;' href='" + path + "&page=" + (beginPage -1) + "'>◀</a>&nbsp;&nbsp;");
			} else {
				sb.append("<a style='text-decoration:none; color: lightgray;' href='" + path + "?page=" + (beginPage -1) + "'>◀</a>&nbsp;&nbsp;");				
			}
		}
		 ...
		 * 		1) 페이지 번호의 링크가 필요 없는 경우 : page == 현재 페이지
		 * 		2) 페이지 번호의 링크가 필요한 경우 : 그 외의 경우
		 */
		for(int p = beginPage; p <= endPage; p++) {
			if(p == page) {
				...
				if(path.indexOf("?") > 0) {
					sb.append("<a style='text-decoration:none; color: lightgray;' href='" + path + "&page=" + p + "'>" + p + "</a>&nbsp;&nbsp;");
				} else {
					sb.append("<a style='text-decoration:none; color: lightgray;' href='" + path + "?page=" + p + "'>" + p + "</a>&nbsp;&nbsp;");					
				}
			}
		 ...
		 *  	1) 다음 버튼의 링크가 필요없는 경우 : endPage == totalPage
		 *  	2) 다음 버튼의 링크가 필요한 경우 : 그 외의 경우
		 */
		if(endPage == totalPage) {
			sb.append("<span style='color: lightgray;'>▶</span>");
		} else {
			if(path.indexOf("?") > 0) {
				sb.append("<a style='text-decoration:none; color: lightgray;' href='" + path + "&page=" + (endPage + 1) + "'>▶</a>");
			} else {
				sb.append("<a style='text-decoration:none; color: lightgray;' href='" + path + "?page=" + (endPage + 1) + "'>▶</a>");				
			}
		}
